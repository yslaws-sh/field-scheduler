...
def load_data_from_google():
    try:
        ...
        if res.status_code == 200:
            ...
            # המרת עמודת הזמן לפורמט תאריך תקין
            df.iloc[:, 0] = pd.to_datetime(df.iloc[:, 0], dayfirst=True, errors='coerce')
            
            # סינון שורות שהן לפני זמן האיפוס
...
if os.path.exists(file_path):
    ...
    with tabs[0]:
        st.subheader(f"שבוע: {st.session_state.start_date.strftime('%d/%m/%Y')}")
    ...
    with tabs[1]:
        if st.text_input("סיסמת מנהל:", type="password") == "1906":
            ...
            if not raw_data.empty:
                ...
                # יצירת טבלה עם סדר עמודות קבוע (ראשון עד חמישי)
                ...
                st.download_button("📥 הורד אקסל", output.getvalue(), f"schedule_{st.session_state.start_date.strftime('%d_%m')}.xlsx")
            else:
                st.info("אין נתונים להצגה. המערכת ריקה או מאופסת.")
